...
public interface InterventionRepository extends JpaRepository<Intervention, Long> {
    ...
    // 🚀 L'FIX HNA : LIMIT 200 bach njebdo 200 w n9esmoha 3la 10 dyal les threads
    @Query(value = "SELECT * FROM interventions WHERE detail_intervention IS NULL OR detail_intervention = '[]' OR detail_intervention = '' ORDER BY id DESC LIMIT 200", nativeQuery = true)
    List<Intervention> findInterventionsWithMissingDetailsDesc();

    // 🚀 L'FIX HNA : LIMIT 200
    @Query(value = "SELECT * FROM interventions WHERE detail_intervention IS NULL OR detail_intervention = '[]' OR detail_intervention = '' ORDER BY id ASC LIMIT 200", nativeQuery = true)
